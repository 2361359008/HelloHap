...
CONTAINER_PROJECT="/root/HAP-BuildKit/minesweeper-project"
CONTAINER_SIGNING="/root/HAP-BuildKit/minesweeper-signing"
...
BUNDLE_NAME="com.openclaw.minesweeper"
ABILITY_NAME="EntryAbility"
UNSIGNED_HAP="$BASE_DIR/minesweeper-unsigned.hap"
SIGNED_HAP="$BASE_DIR/minesweeper-signed.hap"
...
[ -d "$PROJECT_DIR" ] || fail "工程不存在：$PROJECT_DIR"
...
[ -n "$CONTAINER_PYTHON" ] || fail "容器内找不到 Python 3"
...
run_docker exec "$CONTAINER" grep -q "\"bundleName\"[[:space:]]*:[[:space:]]*\"$BUNDLE_NAME\"" \
  "$CONTAINER_PROJECT/AppScope/app.json5" || fail "bundleName 不匹配：$BUNDLE_NAME"
...
run_docker exec "$CONTAINER" \
  ...
  -keystorePwd 123456 \
  -outFile "$CONTAINER_SIGNING/minesweeper-profile.p7b"

log "5/7 签名 HAP"
CONTAINER_SIGNED="$CONTAINER_PROJECT/entry/build/default/outputs/default/minesweeper-signed.hap"
...
run_docker exec "$CONTAINER" \
  ...
  -profileFile "$CONTAINER_SIGNING/minesweeper-profile.p7b" \
...
echo "$INSTALL_OUTPUT"
echo "$INSTALL_OUTPUT" | grep -q "success" || fail "安装未返回成功"
...
if ps -ef | grep "$BUNDLE_NAME" | grep -v grep >/dev/null 2>&1; then
  RUN_PID="$(ps -ef | grep "$BUNDLE_NAME" | grep -v grep | head -n 1 | awk '{print $2}')"
  echo "运行中：$BUNDLE_NAME (PID $RUN_PID)"
else
  fail "应用未运行：$BUNDLE_NAME"
fi
