#!/system/bin/sh
#
# 随心（空白）HAP：编译 + 签名 + 安装 + 启动（与 build_minesweeper.sh 对称，仅路径/包名不同）。
set -eu
...
CONTAINER_PROJECT="/root/HAP-BuildKit/blank-project"
CONTAINER_SIGNING="/root/HAP-BuildKit/blank-signing"
...
BUNDLE_NAME="com.openclaw.blankhap"
ABILITY_NAME="EntryAbility"
UNSIGNED_HAP="$BASE_DIR/blank-unsigned.hap"
SIGNED_HAP="$BASE_DIR/blank-signed.hap"
...
run_docker exec "$CONTAINER" \
  ...
  -keystorePwd 123456 \
  -outFile "$CONTAINER_SIGNING/blank-profile.p7b"

log "5/7 签 HAP"
CONTAINER_SIGNED="$CONTAINER_PROJECT/entry/build/default/outputs/default/blank-signed.hap"
...
run_docker exec "$CONTAINER" \
  ...
  -profileFile "$CONTAINER_SIGNING/blank-profile.p7b" \
