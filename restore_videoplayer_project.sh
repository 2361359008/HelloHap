#!/system/bin/sh
# 多元开发·视频播放器：把板端可编辑工程还原到初始基线（从 project.tar 解出覆盖 project/）。
# 与 restore_minesweeper_project.sh 同结构，仅路径不同。
set -eu

ROOT=/data/local/tmp/videoplayer-hapbuild
...
if ! mkdir "$LOCK" 2>/dev/null; then
  echo "error: videoplayer project restore is already running"
  exit 1
fi
...
if ! mv "$RESTORED" "$PROJECT"; then
  ...
  echo "error: failed to replace the videoplayer HAP project"
  exit 1
fi

rm -rf "$PREVIOUS"

echo "ok: videoplayer HAP project restored from initial baseline"
